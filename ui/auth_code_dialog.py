...
class AuthCodeDialog(QDialog):
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel("鲲穹AI工具箱・软件授权验证", shell)
        title.setStyleSheet(
            """
            font-size: 18px;
            font-weight: 600;
        ...
        self.verify_btn = ModernButton("验证并激活", ModernButton.Style.Primary, shell)
    ...
    def _set_busy(self, busy: bool) -> None:
        ...
        self.verify_btn.setText("正在验证..." if busy else "验证并激活")
    ...
    def _verify(self) -> None:
        ...
        try:
            ...
            if result.success and result.is_valid:
                self.accept()
                return
            self._show_error(result.message or "授权码无效或已过期")
        finally:
    ...
    def _open_auth_code_page(self) -> None:
        try:
            ...
            self._show_error(f"打开授权码页面失败: {exc}")
